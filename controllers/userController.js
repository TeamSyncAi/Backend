...
export async function createAccountUser(req, res) {
  try {
    ...
    const validRoles = ["guest", "teamleader"];
    if (!validRoles.includes(role.toLowerCase())) {
      return res
        .status(400)
        .json({
          error: 'Invalid role. Must be either "guest" or "teamleader".',
        });
    }

    // Create new user with the validated role
    const newUser = await User.create({
      username: req.body.username,
      email: req.body.email,
      password: req.body.password,
      role: role.charAt(0).toUpperCase() + role.slice(1).toLowerCase(), // Capitalize the first letter of the role
    });
...
export async function getUserIdByEmail(req, res) {
  try {
    ...
    if (user && user._id) {
      ...
      res
        .status(404)
        .json({ error: "User not found or does not have a userId" });
    }
  } catch (error) {
    console.error(error);
    ...
    try {
      ...
      if (authenticatedusername !== req.body.username) {
        return res
          .status(403)
          .json({
            error: "Permission denied. You can only change your own picture.",
          });
      }
